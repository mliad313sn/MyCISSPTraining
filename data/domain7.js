...
window.CISSP_DATA.domains[7] = {
  ...
  quiz: [
    ...
    ,
    {"q": "Votre PRA prévoit un basculement vers un site chaud avec un RTO de 4 h. Lors d'un exercice réel de bascule, la restauration technique réussit en 3 h, mais les équipes métier découvrent que les procédures opérationnelles ne couvrent pas la ressaisie des transactions des 45 dernières minutes (RPO), bloquant la reprise pendant 6 h supplémentaires. Quelle est la MEILLEURE conclusion à porter au comité de pilotage ?", "choix": ["L'exercice est un succès : le RTO technique de 4 h est respecté et l'infrastructure a basculé sans incident", "Le RPO doit être réduit à zéro en investissant dans une réplication synchrone pour éliminer toute ressaisie", "L'exercice révèle un écart entre reprise technique et reprise métier : les procédures de continuité d'activité doivent intégrer la reconstitution des données dans le périmètre testé, et le RTO global doit être re-mesuré de bout en bout", "Les équipes métier doivent être formées à ressaisir plus vite pour tenir le RTO"], "reponse": 2, "explication": "Le RTO qui compte est celui de la reprise du SERVICE métier, pas de la seule infrastructure. L'exercice a précisément rempli son rôle : révéler que le plan technique et les procédures métier ne sont pas alignés. La conclusion managériale est de corriger le périmètre du plan et de re-mesurer le RTO réel de bout en bout.", "pourquoi": ["Piège de l'indicateur partiel : déclarer le succès sur le seul RTO technique masque une indisponibilité métier réelle de 9 h — c'est exactement l'angle mort que l'exercice devait révéler.", "Techniquement vrai mais disproportionné : la réplication synchrone répond à un problème de procédures par un investissement lourd, sans analyse coût/bénéfice ni validation que le RPO actuel est inacceptable pour le métier.", "Correct : la bonne conclusion relie reprise technique et reprise métier, corrige le plan et ré-évalue l'objectif de bout en bout — la lecture d'un manager de la continuité.", "Trop étroit : « ressaisir plus vite » traite le symptôme par la pression sur les équipes au lieu de corriger le défaut structurel du plan."], "difficulte": 3},
    {"q": "Une attaque détruit simultanément la production et les sauvegardes en ligne de votre filiale (compte d'administration compromis). Seules les bandes hors ligne mensuelles, stockées à 200 km, subsistent — avec jusqu'à 30 jours de perte de données. La direction exige que cela ne se reproduise « jamais ». Quelle recommandation structurelle est PRIORITAIRE ?", "choix": ["Passer à des sauvegardes horaires répliquées en temps réel sur un second datacenter connecté", "Mettre en place une architecture de sauvegarde 3-2-1 renforcée par une copie immuable (WORM) ou air-gappée, avec des comptes de sauvegarde isolés de l'administration de production et des tests de restauration réguliers", "Chiffrer toutes les sauvegardes pour empêcher l'attaquant de les lire", "Souscrire une cyber-assurance couvrant la reconstruction des données"], "reponse": 1, "explication": "Le facteur aggravant est que les sauvegardes en ligne partageaient la surface d'attaque de la production (même plan d'administration). La réponse structurelle : immuabilité/air gap, séparation des identités d'administration de sauvegarde, et tests de restauration — le cœur de la résilience anti-ransomware.", "pourquoi": ["Piège du « plus fréquent = mieux » : une réplication temps réel connectée aurait été détruite elle aussi — elle réduit le RPO mais pas le risque de destruction simultanée, qui est le problème posé.", "Correct : immuabilité/air gap + isolation des comptes de sauvegarde + tests répondent exactement au mode de défaillance observé.", "Hors sujet : le chiffrement protège la confidentialité des sauvegardes, pas leur disponibilité face à une destruction volontaire.", "Transfert de risque incomplet : l'assurance indemnise financièrement mais ne restaure ni les données ni l'activité — elle complète, elle ne remplace pas la résilience."], "difficulte": 3}
  ],
