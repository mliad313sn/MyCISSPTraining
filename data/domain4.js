...
window.CISSP_DATA.domains[4] = {
  ...
  quiz: [
    ...
    ,
    {"q": "Après une fusion, vous devez interconnecter deux réseaux d'entreprise qui utilisent tous deux 10.0.0.0/8 avec des plans d'adressage qui se chevauchent largement. La direction veut des échanges applicatifs opérationnels en six semaines. Quelle approche recommandez-vous EN PREMIER ?", "choix": ["Interconnexion contrôlée via NAT double sens sur des passerelles dédiées, limitée aux flux applicatifs identifiés, le temps de planifier la convergence d'adressage", "Renuméroter immédiatement le réseau de la société acquise pour éliminer le chevauchement", "Fusionner les domaines d'administration réseau des deux entités pour simplifier la gestion", "Établir un VPN site à site standard entre les deux cœurs de réseau"], "reponse": 0, "explication": "Le chevauchement d'adressage rend le routage direct impossible : le NAT double sens sur des points de passage contrôlés répond au besoin métier dans le délai, en limitant l'exposition aux seuls flux nécessaires — et laisse le temps de traiter la renumérotation comme un projet.", "pourquoi": ["Correct : solution de transition maîtrisée qui respecte le délai, minimise la surface d'interconnexion et n'hypothèque pas la cible.", "Techniquement juste mais irréaliste dans le délai : renuméroter un /8 exploité est un projet de plusieurs mois à fort risque d'interruption.", "Dangereux et prématuré : fusionner l'administration avant d'avoir évalué la posture de sécurité de l'entité acquise étend la surface de compromission.", "Inopérant : un VPN n'élimine pas le conflit d'adressage — les routes se chevauchent toujours."], "difficulte": 3},
    {"q": "Votre SOC signale que l'inspection TLS du proxy sortant est aveugle sur un service SaaS critique depuis que celui-ci impose le certificate pinning. L'équipe réseau propose de placer le domaine en liste d'exemption globale. Quelle est la MEILLEURE réponse ?", "choix": ["Accepter l'exemption mais la compenser : restreindre l'exemption au strict domaine concerné, renforcer les contrôles alternatifs (télémétrie endpoint, CASB via API, journaux du SaaS) et documenter le risque résiduel accepté", "Refuser l'exemption et bloquer le service jusqu'à ce que l'éditeur retire le pinning", "Désactiver l'inspection TLS globalement puisqu'elle devient inefficace", "Installer l'autorité de certification interne sur les serveurs de l'éditeur SaaS"], "reponse": 0, "explication": "Le pinning empêche l'interception licite comme illicite : la réponse mature est une exemption minimale assortie de contrôles compensatoires (visibilité par l'endpoint et l'API du SaaS) et d'une acceptation de risque documentée.", "pourquoi": ["Correct : exemption minimale + contrôles compensatoires + risque documenté — l'arbitrage proportionné d'un manager.", "Absolu contre-productif : bloquer un service critique pour un point de visibilité inverse la hiérarchie risque/besoin métier.", "Généralisation abusive : renoncer à toute inspection parce qu'un domaine y échappe détruit un contrôle qui reste efficace ailleurs.", "Techniquement absurde : on n'installe pas sa CA chez un tiers — c'est méconnaître le fonctionnement du pinning."], "difficulte": 3},
    {"q": "Un site industriel connecte ses automates (OT) au réseau IT pour remonter des données de production vers un outil d'analyse cloud. L'intégrateur propose une règle de pare-feu autorisant le serveur d'historisation, situé dans la zone OT, à pousser directement vers Internet. Quelle exigence d'architecture imposez-vous ?", "choix": ["Le passage par une zone démilitarisée industrielle (iDMZ) : l'historian OT pousse vers un réplica en iDMZ, seul ce réplica dialogue avec le cloud — aucun flux direct OT→Internet", "Le chiffrement TLS du flux direct entre l'historian OT et le cloud", "Une authentification forte de l'outil cloud avant chaque collecte", "La limitation du débit du flux sortant pour éviter l'exfiltration massive"], "reponse": 0, "explication": "Le modèle Purdue/IEC 62443 impose la rupture de flux en iDMZ : la zone OT ne dialogue jamais directement avec Internet. Le réplica en iDMZ absorbe l'exposition ; TLS et authentification s'ajoutent mais ne remplacent pas la ségrégation.", "pourquoi": ["Correct : la rupture de zone (iDMZ) est l'exigence structurelle — elle évite qu'une compromission cloud/Internet touche directement la zone de contrôle industriel.", "Techniquement vrai mais insuffisant : TLS protège le transport, pas la topologie — un canal chiffré direct reste un chemin d'attaque vers l'OT.", "Contrôle utile mais secondaire : l'authentification ne compense pas une exposition directe de la zone OT.", "Trop étroit : limiter le débit gêne l'exfiltration mais n'empêche ni l'intrusion ni la latéralisation vers les automates."], "difficulte": 3},
    {"q": "Une PME dont le site e-commerce subit un DDoS applicatif (L7) sous une adresse IP directement exposée envisage trois offres : un service anti-DDoS BGP de son opérateur, un CDN avec WAF intégré, ou le doublement de la capacité de ses serveurs. Le budget ne permet qu'une option. Laquelle est la PLUS adaptée ?", "choix": ["Le CDN avec WAF : il masque l'origine, absorbe la charge en périphérie et filtre les requêtes applicatives — le vecteur exact de l'attaque", "Le service anti-DDoS BGP de l'opérateur, référence contre les attaques volumétriques", "Le doublement de la capacité serveur, qui améliore aussi les performances au quotidien", "Les trois sont équivalents : choisir la moins chère"], "reponse": 0, "explication": "Une attaque L7 se filtre au niveau applicatif : le CDN/WAF traite les requêtes malveillantes en périphérie et cache l'IP d'origine. La protection BGP excelle contre le volumétrique L3/L4 mais laisse passer du L7 « propre » ; ajouter des serveurs ne fait que renchérir la cible.", "pourquoi": ["Correct : réponse alignée sur le vecteur (applicatif) — filtrage L7 en périphérie + masquage de l'origine.", "Techniquement vrai mais décalé : l'anti-DDoS BGP vise le volumétrique réseau, pas les requêtes applicatives légitimes en apparence.", "Piège du sur-dimensionnement : doubler la capacité ne fait que repousser le seuil — l'attaquant scale plus vite que votre budget serveur.", "Faux : les trois options traitent des couches différentes — les mettre à égalité nie l'analyse du vecteur."], "difficulte": 3}
  ],
